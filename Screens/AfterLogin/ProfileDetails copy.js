...
export const ProfileDetails = () => {
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      ...
      try {
        ...
        // console.log("Profile Details fully ==>", data, response)
        setProfileData(data);
        if (data && data.basic_details.express_int === "1") {
          setExpressInt(true);
        ...
        // Load unlock status and images from AsyncStorage
        const unlockStatus = await AsyncStorage.getItem(`profileUnlocked_${data.basic_details.profile_id}`);
        if (unlockStatus === 'true') {
          setIsProfileUnlocked(true);
          const storedImages = await AsyncStorage.getItem(`fetchedUserImages_${data.basic_details.profile_id}`);
          if (storedImages) {
            setFetchedUserImages(JSON.parse(storedImages));
          }
        }
        // setProfileData(data.data);
        setVysassistEnable(data.basic_details.vysy_assist_enable)
        setVysassits(data.basic_details.vys_assits)
        // setData(data.basic_details.vys_list)
  ...
  useEffect(() => {
    const loadWishlistProfiles = async () => {
      try {
        // console.log('Fetching wishlist profiles...');
        ...
        if (response) {
          ...
          // console.log('Profile IDs Set:', profileIdsSet);
